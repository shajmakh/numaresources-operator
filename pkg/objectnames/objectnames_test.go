...
func TestGetAssociatedNameFromRTEDaemonset(t *testing.T) {
	got := GetAssociatedNameFromRTEDaemonset("bar-foo", "bar")
	if got != "foo" {
		t.Errorf("expected \"foo\" got %s", got)
	}

	got = GetAssociatedNameFromRTEDaemonset("bar-foo-foobar", "bar")
	if got != "foo-foobar" {

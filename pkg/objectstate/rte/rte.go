...
func (em *ExistingManifests) State(mf rtemanifests.Manifests, updater GenerateDesiredManifestUpdater) []objectstate.ObjectState {
	...
	for _, tree := range em.trees {
		if tree.NodeGroup.NodeSelector != nil {
			var existingDs client.Object
			var loadError error

			ngSelector := tree.NodeGroup.NodeSelector

			generatedName := objectnames.GetComponentName(em.instance.Name, *tree.NodeGroup.Name)
			...
			if updater != nil {
				...
				err := updater(*tree.NodeGroup.Name, &gdm)
				if err != nil {
					updateError = fmt.Errorf("daemonset for nodeSelector %q: update failed: %w", *tree.NodeGroup.Name, err)
				}
			}
...
func FromClient(ctx context.Context, cli client.Client, plat platform.Platform, mf rtemanifests.Manifests, instance *nropv1.NUMAResourcesOperator, trees []nodegroupv1.Tree, namespace string) ExistingManifests {
	...
	for _, tree := range trees {
		if tree.NodeGroup.NodeSelector != nil {
			generatedName := objectnames.GetComponentName(instance.Name, *tree.NodeGroup.Name)

...
// exitWithCancel cancels the context then exits. Used so main() can defer cancel()
// without triggering gocritic exitAfterDefer (no os.Exit in the same function as defer).
func exitWithCancel(cancel context.CancelFunc, exitCode int) {
	cancel()
	os.Exit(exitCode)
}
...
func main() {
	...
	if params.renderMode {
		...
		exitWithCancel(cancel, manageRendering(params.render, discoveredCluster.Platform, apiManifests, rteMf, namespace, params.enableScheduler))
	}
	...
	tlsSecurityProfileSpec, err := ctrltls.FetchAPIServerTLSProfile(ctx, k8sClient)
	if err != nil {

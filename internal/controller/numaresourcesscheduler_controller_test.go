...
var _ = Describe("Test NUMAResourcesScheduler Reconcile", func() {
	...
	Context("with kubelet PodResourcesAPI listing active pods by default", func() {
		...
		When("kubelet fix is enabled", func() {
			fixedVersion, _ := platform.ParseVersion("5.0.0")
			unfixedVersion, _ := platform.ParseVersion("4.19.0")           // can't (and we must not even if we can) rewrite history
			futureFixedVersionZstream, _ := platform.ParseVersion("5.0.1") // we must never regress
			futureFixedVersion, _ := platform.ParseVersion("5.1.0")        // we must never regress
	...
	Context("with TLS settings in deployment", func() {
		var nrs *nropv1.NUMAResourcesScheduler
		var nrsKey client.ObjectKey
		var reconciler *NUMAResourcesSchedulerReconciler
		...
		It("should set the TLS arguments in the deployment by default and reflect updates", func() {
	...
	Context("when setting the scheduler PodAntiAffinity", func() {
		...
		expectedSafeStrategy := appsv1.DeploymentStrategy{
		...
		DescribeTable("should set PodAntiAffinity and Rollout Strategy for scheduler Deployment according to replicas count", func(ctx context.Context, tc testCase) {
			nrs := testobjs.NewNUMAResourcesScheduler("numaresourcesscheduler", "some/url:latest", testSchedulerName, 11*time.Second)
			...
			initialStrategy := *reconciler.SchedulerManifests.Deployment.Spec.Strategy.DeepCopy()
			...
			if !tc.expectPodAntiAffinity {
				...
				Expect(dp.Spec.Strategy).To(Equal(initialStrategy))
				return
			}
			...
			Expect(dp.Spec.Strategy).To(Equal(expectedSafeStrategy))
		},
